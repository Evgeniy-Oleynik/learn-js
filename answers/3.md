...
console.log(fruit[fruit.length - 1]);  //  1
...
const fruit = ['Apple', 'Orange', 'Plum'];
fruit.push(PINEAPPLE);  //  1

fruit[fruit.length] = PINEAPPLE;    //  2
```
...
```js
const fruit = ['Apple', 'Orange', 'Plum'];
const otherFruits = [].concat(fruit);   //  1
const otherFruits = fruit.slice();  //  2
...
function checkFruits(fruit) {
    ...
    return console.log(kanon.includes(fruit) ? fruit : null);
}
```
...
const lastFruit = fruit[fruit.length - 1];  //  1
